...
void recur(int n, int base, int target, int aux, vector<vector<int>>& buff) {
  if (n == 0) return;
  recur(n - 1, base, aux, target, buff);
  buff.push_back({base, target});
  recur(n - 1, aux, target, base, buff);
}

vector<vector<int>> ComputeTowerHanoi(int num_rings) {
  vector<vector<int>> result;
  recur(num_rings, 0, 1, 2, result);
  return result;
}
