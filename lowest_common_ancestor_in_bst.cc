...
BstNode<int>* FindLCA(const unique_ptr<BstNode<int>>& tree,
  ...
  int lo = std::min(s->data, b->data);
  int hi = std::max(s->data, b->data);
  
  for (auto node = tree.get(); node;) {
    if (node->data < lo) node = node->right.get();
    else if (node->data > hi) node = node->left.get();
    else return node;
  }
