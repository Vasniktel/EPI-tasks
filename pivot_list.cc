...
shared_ptr<ListNode<int>> ListPivoting(const shared_ptr<ListNode<int>>& l,
                                       int x) {
  auto less = make_shared<ListNode<int>>(), curr_less = less;
  auto equal = make_shared<ListNode<int>>(), curr_equal = equal;
  auto greater = make_shared<ListNode<int>>(), curr_greater = greater;
  ...
  for (auto node = l; node; node = node->next) {
    if (node->data < x) {
      ...
      curr_equal->next = node;
      curr_equal = node;
    } else {
      curr_greater->next = node;
      curr_greater = node;
    }
  }

  curr_greater->next = nullptr;
  curr_equal->next = greater->next;
  curr_less->next = equal->next;
  return less->next;
}
