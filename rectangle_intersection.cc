...
Rect IntersectRectangle(const Rect& r1, const Rect& r2) {
  bool on_x = !(r1.x > r2.x + r2.width || r1.x + r1.width < r2.x);
  bool on_y = !(r1.y > r2.y + r2.height || r1.y + r1.height < r2.y);
  if (!(on_x && on_y)) return {0, 0, -1, -1};
  
  int x = std::max(r1.x, r2.x);
  int y = std::max(r1.y, r2.y);
  int width = std::min(r1.x + r1.width, r2.x + r2.width) - x;
  int height = std::min(r1.y + r1.height, r2.y + r2.height) - y;
